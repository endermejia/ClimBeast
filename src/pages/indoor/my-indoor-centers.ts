      ...
      <section class="w-full max-w-[1600px] mx-auto p-4 pb-32">
        <header class="flex items-center justify-between gap-2">
          ...
          >
            ...
            <span>{{ 'myIndoorCenters.title' | translate }}</span>
          </h1>
        </header>

        <div tuiNotification size="m" class="my-4">
          {{ 'myIndoorCenters.notification' | translate }}
        </div>
        ...
        <div class="sticky top-0 z-10 py-4 flex items-end gap-2">
          <tui-textfield
            ...
            tuiTextfieldSize="l"
          >
            <label tuiLabel for="gyms-search">
            ...
            <input
              ...
              autocomplete="off"
              [value]="query()"
              (input.zoneless)="onQuery(gymsSearch.value)"
            />
          </tui-textfield>
        </div>
  ...
  readonly loading = computed(
    () =>
      ...
      (!this.authState.isAdmin() &&
        (this.authState.adminIndoorCentersResource.isLoading() ||
          this.authState.adminIndoorCentersResource.value() === undefined)),
  );

  readonly query: WritableSignal<string> = signal('');

  readonly myCenterIds = this.authState.adminIndoorCenters;

  readonly centers = computed(() => {
    if (this.authState.isAdmin()) {
      return this.indoorCentersData.indoorCentersList();
    }
    const ids = new Set(this.myCenterIds().map((id) => String(id)));
